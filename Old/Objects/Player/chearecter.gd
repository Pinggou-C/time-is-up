...
func _process(_delta):
	$Camera.rotation_degrees.x = lerp($Camera.rotation_degrees.x, $Camera.rotation_degrees.x - mouseDelta.y*2, 0.333)
	$Camera.rotation_degrees.x = clamp($Camera.rotation_degrees.x, -89, 89) 
	rotation_degrees.y = lerp(rotation_degrees.y, rotation_degrees.y - mouseDelta.x*3, 0.333)
	mouseDelta = Vector2()
